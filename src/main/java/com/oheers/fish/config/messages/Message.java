...
public class Message {
	...
	public void setCompetitionType(@NotNull final CompetitionType type) {
		switch (type) {
			case LARGEST_TOTAL: setVariable("{type", new Message(ConfigMessage.LEADERBOARD_LARGEST_TOTAL).getRawMessage(false, false)); break;
	...
	public void setCustomFishLore(@NotNull final String id) {
		...
		if (potentialLore.size() > 0) {
			...
			for (String line : potentialLore) {
				if (line.equals(potentialLore.get(potentialLore.size()-1))) customLore.append(FishUtils.translateHexColorCodes(line));
